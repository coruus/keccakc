...
int sha3_224(register uint8_t* const restrict out,
           ...
           register const uint8_t* const restrict in,
           register const size_t inlen) {
  do { if (out == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 2916, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (in == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 2917, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (outlen != 28) { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 2918, err_digestlen); return errno_digestlen; } while (0); } } while (0);
  keccak_sponge sponge;
  int err = 0;
  err = sha3_224_init(&sponge);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 2923, err_sha3); return errno_sha3; } while (0); } } while (0);
  ...
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 2927, err_sha3); return errno_sha3; } while (0); } } while (0);
  memclear(&sponge, sizeof(sponge));
  return err;
}
int sha3_256_init(register keccak_sponge* const restrict sponge) {
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 2974, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_init(sponge, 200 - (32 * 2), flag_sha3_256 ^ hash_absorbing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 2977, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_256_update(register keccak_sponge* const restrict sponge,
                  register const uint8_t* const restrict in,
                  register const size_t inlen) {
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3002, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (in == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3003, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_update(sponge, in, inlen, flag_sha3_256 ^ hash_absorbing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3006, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_256_digest(register keccak_sponge* const restrict sponge,
                  register uint8_t* const restrict out,
                  register const size_t outlen) {
  do { if (outlen != 32) { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3030, err_digestlen); return errno_digestlen; } while (0); } } while (0);
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3031, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (out == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3032, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_finalize(
      sponge, pad_sha, flag_sha3_256 ^ hash_absorbing, flag_sha3_256 ^ hash_squeezing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3037, err_sha3); return errno_sha3; } while (0); } } while (0);
  memclear((uint8_t*)sponge->a + 64, 200 - 64);
  err = _hash_squeeze(sponge, out, 32, flag_sha3_256 ^ hash_squeezing);
  memclear(sponge, 32 * 8);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3046, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_256(register uint8_t* const restrict out,
           register const size_t outlen,
           register const uint8_t* const restrict in,
           register const size_t inlen) {
  do { if (out == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3069, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (in == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3070, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (outlen != 32) { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3071, err_digestlen); return errno_digestlen; } while (0); } } while (0);
  keccak_sponge sponge;
  int err = 0;
  err = sha3_256_init(&sponge);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3076, err_sha3); return errno_sha3; } while (0); } } while (0);
  err = sha3_256_update(&sponge, in, inlen);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3078, err_sha3); return errno_sha3; } while (0); } } while (0);
...
int sha3_384_init(register keccak_sponge* const restrict sponge) {
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3127, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_init(sponge, 200 - (48 * 2), flag_sha3_384 ^ hash_absorbing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3130, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_384_update(register keccak_sponge* const restrict sponge,
                  register const uint8_t* const restrict in,
                  register const size_t inlen) {
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3155, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (in == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3156, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_update(sponge, in, inlen, flag_sha3_384 ^ hash_absorbing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3159, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_384_digest(register keccak_sponge* const restrict sponge,
                  register uint8_t* const restrict out,
                  register const size_t outlen) {
  do { if (outlen != 48) { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3183, err_digestlen); return errno_digestlen; } while (0); } } while (0);
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3184, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (out == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3185, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_finalize(
      sponge, pad_sha, flag_sha3_384 ^ hash_absorbing, flag_sha3_384 ^ hash_squeezing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3190, err_sha3); return errno_sha3; } while (0); } } while (0);
  memclear((uint8_t*)sponge->a + 64, 200 - 64);
  err = _hash_squeeze(sponge, out, 48, flag_sha3_384 ^ hash_squeezing);
  memclear(sponge, 32 * 8);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3199, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_384(register uint8_t* const restrict out,
           register const size_t outlen,
           register const uint8_t* const restrict in,
           register const size_t inlen) {
  do { if (out == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3222, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (in == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3223, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (outlen != 48) { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3224, err_digestlen); return errno_digestlen; } while (0); } } while (0);
  keccak_sponge sponge;
  int err = 0;
  err = sha3_384_init(&sponge);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3229, err_sha3); return errno_sha3; } while (0); } } while (0);
  err = sha3_384_update(&sponge, in, inlen);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3231, err_sha3); return errno_sha3; } while (0); } } while (0);
  err = sha3_384_digest(&sponge, out, outlen);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3233, err_sha3); return errno_sha3; } while (0); } } while (0);
  memclear(&sponge, sizeof(sponge));
  return err;
}
int sha3_512_init(register keccak_sponge* const restrict sponge) {
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3280, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_init(sponge, 200 - (64 * 2), flag_sha3_512 ^ hash_absorbing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3283, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_512_update(register keccak_sponge* const restrict sponge,
                  register const uint8_t* const restrict in,
                  register const size_t inlen) {
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3308, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (in == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3309, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_update(sponge, in, inlen, flag_sha3_512 ^ hash_absorbing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3312, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_512_digest(register keccak_sponge* const restrict sponge,
                  register uint8_t* const restrict out,
                  register const size_t outlen) {
  do { if (outlen != 64) { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3336, err_digestlen); return errno_digestlen; } while (0); } } while (0);
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3337, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (out == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3338, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_finalize(
      sponge, pad_sha, flag_sha3_512 ^ hash_absorbing, flag_sha3_512 ^ hash_squeezing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3343, err_sha3); return errno_sha3; } while (0); } } while (0);
  memclear((uint8_t*)sponge->a + 64, 200 - 64);
  err = _hash_squeeze(sponge, out, 64, flag_sha3_512 ^ hash_squeezing);
  memclear(sponge, 32 * 8);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3352, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_512(register uint8_t* const restrict out,
           register const size_t outlen,
           register const uint8_t* const restrict in,
           register const size_t inlen) {
  do { if (out == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3375, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (in == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3376, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (outlen != 64) { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3377, err_digestlen); return errno_digestlen; } while (0); } } while (0);
  keccak_sponge sponge;
  int err = 0;
  err = sha3_512_init(&sponge);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3382, err_sha3); return errno_sha3; } while (0); } } while (0);
  ...
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3386, err_sha3); return errno_sha3; } while (0); } } while (0);
  memclear(&sponge, sizeof(sponge));
  return err;
}

...
int keccak_sponge_absorb(register keccak_sponge* const restrict sponge,
  ...
  if ((in == ((void *)0)) || (inlen > (4294967295UL >> 1))) {
    do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 99, err_rsize); return errno_rsize; } while (0);
  }
  do { int err = _sponge_checkinv(sponge); if (err != 0) { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 101, err_sponge_invariant); return errno_sponge_invariant; } while (0); } } while (0);
...
int keccak_sponge_squeeze(register keccak_sponge* const restrict sponge,
  ...
  if ((out == ((void *)0)) || (outlen > (4294967295UL >> 1))) {
    do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 114, err_rsize); return errno_rsize; } while (0);
  }
  do { int err = _sponge_checkinv(sponge); if (err != 0) { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 116, err_sponge_invariant); return errno_sponge_invariant; } while (0); } } while (0);
...
static inline int _hash_update(register keccak_sponge* const restrict sponge,
  ...
  if (sponge->flags != flags) {
    do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "keccak/modes/hash/hash-impl.h", 29, err_hash_flags); return errno_hash_flags; } while (0);
  }
...
static inline int _hash_finalize(register keccak_sponge* const restrict sponge,
  ...
  if (err != 0) {
    ...
    do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "keccak/modes/hash/hash-impl.h", 42, err_hash_finalized); return errno_hash_finalized; } while (0);
  } else if (sponge->flags != oldflags) {
    do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "keccak/modes/hash/hash-impl.h", 44, err_hash_flags); return errno_hash_flags; } while (0);
  }
...
static inline int _hash_squeeze(register keccak_sponge* const restrict sponge,
  ...
  if (err != 0) {
    ...
    do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "keccak/modes/hash/hash-impl.h", 78, err_hash_not_finalized); return errno_hash_not_finalized; } while (0);
  } else if (sponge->flags != flags) {
    do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "keccak/modes/hash/hash-impl.h", 80, err_hash_flags); return errno_hash_flags; } while (0);
  }
  return keccak_sponge_squeeze(sponge, out, outlen);
}
int sha3_224_init(register keccak_sponge* const restrict sponge) {
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 2813, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_init(sponge, 200 - (28 * 2), flag_sha3_224 ^ hash_absorbing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 2816, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_224_update(register keccak_sponge* const restrict sponge,
                  register const uint8_t* const restrict in,
                  register const size_t inlen) {
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 2841, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (in == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 2842, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_update(sponge, in, inlen, flag_sha3_224 ^ hash_absorbing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 2845, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_224_digest(register keccak_sponge* const restrict sponge,
                  register uint8_t* const restrict out,
                  register const size_t outlen) {
  do { if (outlen != 28) { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 2876, err_digestlen); return errno_digestlen; } while (0); } } while (0);
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 2877, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (out == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 2878, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_finalize(
      sponge, pad_sha, flag_sha3_224 ^ hash_absorbing, flag_sha3_224 ^ hash_squeezing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 2883, err_sha3); return errno_sha3; } while (0); } } while (0);
  memclear((uint8_t*)sponge->a + 64, 200 - 64);
  err = _hash_squeeze(sponge, out, 28, flag_sha3_224 ^ hash_squeezing);
  memclear(sponge, 32 * 8);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 2891, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_224(register uint8_t* const restrict out,
           register const size_t outlen,
           register const uint8_t* const restrict in,
           register const size_t inlen) {
  do { if (out == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 2914, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (in == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 2915, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (outlen != 28) { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 2916, err_digestlen); return errno_digestlen; } while (0); } } while (0);
  keccak_sponge sponge;
  int err = 0;
  err = sha3_224_init(&sponge);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 2922, err_sha3); return errno_sha3; } while (0); } } while (0);
  err = sha3_224_update(&sponge, in, inlen);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 2925, err_sha3); return errno_sha3; } while (0); } } while (0);
  err = sha3_224_digest(&sponge, out, outlen);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 2928, err_sha3); return errno_sha3; } while (0); } } while (0);
  memclear(&sponge, sizeof(sponge));
  return err;
}
int sha3_256_init(register keccak_sponge* const restrict sponge) {
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 2965, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_init(sponge, 200 - (32 * 2), flag_sha3_256 ^ hash_absorbing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 2968, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_256_update(register keccak_sponge* const restrict sponge,
                  register const uint8_t* const restrict in,
                  register const size_t inlen) {
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 2993, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (in == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 2994, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_update(sponge, in, inlen, flag_sha3_256 ^ hash_absorbing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 2997, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_256_digest(register keccak_sponge* const restrict sponge,
                  register uint8_t* const restrict out,
                  register const size_t outlen) {
  do { if (outlen != 32) { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3028, err_digestlen); return errno_digestlen; } while (0); } } while (0);
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3029, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (out == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3030, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_finalize(
      sponge, pad_sha, flag_sha3_256 ^ hash_absorbing, flag_sha3_256 ^ hash_squeezing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3035, err_sha3); return errno_sha3; } while (0); } } while (0);
  memclear((uint8_t*)sponge->a + 64, 200 - 64);
  err = _hash_squeeze(sponge, out, 32, flag_sha3_256 ^ hash_squeezing);
  memclear(sponge, 32 * 8);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3043, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_256(register uint8_t* const restrict out,
           register const size_t outlen,
           register const uint8_t* const restrict in,
           register const size_t inlen) {
  do { if (out == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3066, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (in == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3067, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (outlen != 32) { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3068, err_digestlen); return errno_digestlen; } while (0); } } while (0);
  keccak_sponge sponge;
  int err = 0;
  err = sha3_256_init(&sponge);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3074, err_sha3); return errno_sha3; } while (0); } } while (0);
  err = sha3_256_update(&sponge, in, inlen);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3077, err_sha3); return errno_sha3; } while (0); } } while (0);
  err = sha3_256_digest(&sponge, out, outlen);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3080, err_sha3); return errno_sha3; } while (0); } } while (0);
  memclear(&sponge, sizeof(sponge));
  return err;
}
int sha3_384_init(register keccak_sponge* const restrict sponge) {
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3117, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_init(sponge, 200 - (48 * 2), flag_sha3_384 ^ hash_absorbing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3120, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_384_update(register keccak_sponge* const restrict sponge,
                  register const uint8_t* const restrict in,
                  register const size_t inlen) {
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3145, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (in == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3146, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_update(sponge, in, inlen, flag_sha3_384 ^ hash_absorbing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3149, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_384_digest(register keccak_sponge* const restrict sponge,
                  register uint8_t* const restrict out,
                  register const size_t outlen) {
  do { if (outlen != 48) { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3180, err_digestlen); return errno_digestlen; } while (0); } } while (0);
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3181, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (out == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3182, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_finalize(
      sponge, pad_sha, flag_sha3_384 ^ hash_absorbing, flag_sha3_384 ^ hash_squeezing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3187, err_sha3); return errno_sha3; } while (0); } } while (0);
  memclear((uint8_t*)sponge->a + 64, 200 - 64);
  err = _hash_squeeze(sponge, out, 48, flag_sha3_384 ^ hash_squeezing);
  memclear(sponge, 32 * 8);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3195, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_384(register uint8_t* const restrict out,
           register const size_t outlen,
           register const uint8_t* const restrict in,
           register const size_t inlen) {
  do { if (out == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3218, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (in == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3219, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (outlen != 48) { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3220, err_digestlen); return errno_digestlen; } while (0); } } while (0);
  keccak_sponge sponge;
  int err = 0;
  err = sha3_384_init(&sponge);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3226, err_sha3); return errno_sha3; } while (0); } } while (0);
  err = sha3_384_update(&sponge, in, inlen);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3229, err_sha3); return errno_sha3; } while (0); } } while (0);
  err = sha3_384_digest(&sponge, out, outlen);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3232, err_sha3); return errno_sha3; } while (0); } } while (0);
  memclear(&sponge, sizeof(sponge));
  return err;
}
int sha3_512_init(register keccak_sponge* const restrict sponge) {
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3269, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_init(sponge, 200 - (64 * 2), flag_sha3_512 ^ hash_absorbing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3272, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_512_update(register keccak_sponge* const restrict sponge,
                  register const uint8_t* const restrict in,
                  register const size_t inlen) {
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3297, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (in == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3298, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_update(sponge, in, inlen, flag_sha3_512 ^ hash_absorbing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3301, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_512_digest(register keccak_sponge* const restrict sponge,
                  register uint8_t* const restrict out,
                  register const size_t outlen) {
  do { if (outlen != 64) { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3332, err_digestlen); return errno_digestlen; } while (0); } } while (0);
  do { if (sponge == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3333, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (out == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3334, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  int err = _hash_finalize(
      sponge, pad_sha, flag_sha3_512 ^ hash_absorbing, flag_sha3_512 ^ hash_squeezing);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3339, err_sha3); return errno_sha3; } while (0); } } while (0);
  memclear((uint8_t*)sponge->a + 64, 200 - 64);
  err = _hash_squeeze(sponge, out, 64, flag_sha3_512 ^ hash_squeezing);
  memclear(sponge, 32 * 8);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3347, err_sha3); return errno_sha3; } while (0); } } while (0);
  return 0;
}
int sha3_512(register uint8_t* const restrict out,
           register const size_t outlen,
           register const uint8_t* const restrict in,
           register const size_t inlen) {
  do { if (out == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3370, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (in == ((void *)0)) { do { fprintf(__stderrp, "%s:%u: hard rte: %s\n", "<stdin>", 3371, err_ptrnull); return errno_ptrnull; } while (0); } } while (0);
  do { if (outlen != 64) { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3372, err_digestlen); return errno_digestlen; } while (0); } } while (0);
  keccak_sponge sponge;
  int err = 0;
  err = sha3_512_init(&sponge);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3378, err_sha3); return errno_sha3; } while (0); } } while (0);
  err = sha3_512_update(&sponge, in, inlen);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3381, err_sha3); return errno_sha3; } while (0); } } while (0);
  err = sha3_512_digest(&sponge, out, outlen);
  do { if (err == 0) { break; } else { do { fprintf(__stderrp, "%s:%u: soft rte: %s\n", "<stdin>", 3384, err_sha3); return errno_sha3; } while (0); } } while (0);
  memclear(&sponge, sizeof(sponge));
  return err;
}
...
static const uint8_t sha3_512_len0[64] = {0xa6,
                                          ...
                                          0xcd,
                                          0x26};
int test_sha3_224_len0(void); int test_sha3_224_len0(void) { keccak_sponge sponge; uint8_t out[(224 / 8)] = {0}; int err = 0; err |= sha3_224_init(&sponge); err |= sha3_224_digest(&sponge, out, (224 / 8)); printkat(((void *)0), 0, out, (224 / 8)); return err | checktest("SHA3-" "224" "('')", sha3_224_len0, out, (224 / 8)); } int test_sha3_256_len0(void); int test_sha3_256_len0(void) { keccak_sponge sponge; uint8_t out[(256 / 8)] = {0}; int err = 0; err |= sha3_256_init(&sponge); err |= sha3_256_digest(&sponge, out, (256 / 8)); printkat(((void *)0), 0, out, (256 / 8)); return err | checktest("SHA3-" "256" "('')", sha3_256_len0, out, (256 / 8)); } int test_sha3_384_len0(void); int test_sha3_384_len0(void) { keccak_sponge sponge; uint8_t out[(384 / 8)] = {0}; int err = 0; err |= sha3_384_init(&sponge); err |= sha3_384_digest(&sponge, out, (384 / 8)); printkat(((void *)0), 0, out, (384 / 8)); return err | checktest("SHA3-" "384" "('')", sha3_384_len0, out, (384 / 8)); } int test_sha3_512_len0(void); int test_sha3_512_len0(void) { keccak_sponge sponge; uint8_t out[(512 / 8)] = {0}; int err = 0; err |= sha3_512_init(&sponge); err |= sha3_512_digest(&sponge, out, (512 / 8)); printkat(((void *)0), 0, out, (512 / 8)); return err | checktest("SHA3-" "512" "('')", sha3_512_len0, out, (512 / 8)); } int main(void) {
  int err = 0;
  do { err |= test_sha3_224_len0(); ((void)0); } while (0);
  do { err |= test_sha3_256_len0(); ((void)0); } while (0);
  do { err |= test_sha3_384_len0(); ((void)0); } while (0);
  do { err |= test_sha3_512_len0(); ((void)0); } while (0);
  return err;
}

#!/usr/bin/env sh
s="keccak/constructions/sponge.c keccak/keccak/keccakf.c keccak/fips202/sha3/sha3_*.c keccak/fips202/shake/shake*.c"
...
parallel echo build '\$\{b\}'/{1/.}-{2/.}: cexec {1} {2} build/libclear.dylib $s \
  ::: ${impls} \
  ::: $t/test0-sha3.c $t/test0-shake.c $t/test-kats.c > ninjas/buildlines.ninja
parallel echo build '\$\{b\}/bench-\$\{cc\}'-{1/.}-{2/.}: cexec_optim build/libutil.dylib {1} {2} $s  \
  ::: ${impls} \
